          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            >
              ...
              <motion.div variants={fadeUp} className="flex flex-wrap items-center gap-3 mb-6">
                <span className="flex items-center gap-2 text-sm text-text-secondary bg-subtle/5 px-3 py-1.5 rounded-full">
              ...
              <motion.div variants={fadeUp} className="flex flex-wrap gap-3 mb-8">
                ...
                >
                  ...
                  {t("release.listenOnSpotify")}
                </a>
                {release.purchase && <BuyDigitalButton release={release} />}
              </motion.div>
                ...
                <motion.div variants={fadeUp} className="mt-8">
                  ...
                  <ol className="space-y-1">
                    {release.tracklist.map((track) => {
                      ...
                      const inner = (
                        <>
                          <span className={`text-sm w-6 text-right transition-colors ${hasLyrics ? "text-muted group-hover:text-[#E8385D]" : "text-muted/50"}`}>
                            {track.number}
                          </span>
                          <Music size={14} className={`transition-colors ${hasLyrics ? "text-[#E8385D]/50 group-hover:text-[#E8385D]" : "text-muted/30"}`} />
                          <span className={`text-sm flex-1 ${hasLyrics ? "text-foreground" : "text-foreground/70"}`}>
                            {track.title}
                            {track.feat && (
                          ...
                          {hasLyrics && (
                            <span className="text-xs uppercase tracking-wider text-[#E8385D]/60 font-semibold">{t("release.lyrics")}</span>
                          )}
                          {track.duration && (
              ...
              {release.credits && release.credits.length > 0 && (
                <motion.div variants={fadeUp} className="mt-8">
                  <h2 className="text-lg font-bold text-foreground mb-4">{t("release.credits")}</h2>
